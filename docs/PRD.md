...
|---|------|------|----------|-------------|
| F1 | 도트 방 메인 | 정서적 재방문 보상 | 5개 hitbox 렌더·이동, 곰 idle | 다수 방 전환, 시즌 테마 |
| F2 | 책 등록 | 내 책장 구축 | 바코드/검색 탭, 알라딘 프록시 | 수기 등록, CSV 임포트 |
| F3 | 책장 | 시각적 책 탐색 | 표지 그리드, 빈 상태 | 태그·정렬 필터, 폴더 |
| F4 | 독서 세션 | 날짜·페이지 진행 기록 | 날짜/페이지/분, 해당 책 diary 링크 | 타이머, 목표 진도 |
